...
$("#DoctorAvailabilityTimings-0").change(function() {
...
function getTimeFields(id){
	
    var select1 = $("#timeDDn1"+id);
	var select2 = $("#timeDDn2"+id);
    ...
    for(var i = 60; i <= 1440; i += 15){
        ...
        select1.append($('<option></option>').attr('value', hours + ':' + minutes + ' ' + ampm).text(hours + ':' + minutes + ' ' + ampm)); 
		select2.append($('<option></option>').attr('value', hours + ':' + minutes + ' ' + ampm).text(hours + ':' + minutes + ' ' + ampm)); 
    }
...
var x='200';
function add_values(id=null , val=null){

	$('#DoctorAvailabilityTimings_'+id+'_chzn .chzn-choices .search-field').remove();
	$('#DoctorAvailabilityTimings_'+id+'_chzn .chzn-choices').append('<li id="DoctorAvailabilityTimings_'+id+'_chzn_c_'+x+'" class="search-choice"><span>'+val+'</span><a  class="search-choice-close" rel="'+x+'" onclick="get_removed(obj)"></a></li>');
x++;

var myOptions = {
    val : val
};
var mySelect = $('#DoctorAvailabilityTimings-'+id);
$.each(myOptions, function(text, text) {
    mySelect.append(
