	...
	function generate ($data, $settings = array ()) {
		...
		foreach ($data as $i => $result) {
			...
			if ($this->__settings['show_add_role']) {
				$return .= ' ';
				$return .= $this->Html->link(__l('Add Role'), array('controller' => 'roles', 'action' => 'add', $result[$model]['id']), array('class' => 'add', 'title' => __l('Add Role')));
			}
			if ($this->__settings['show_edit_role']) {
				$return .= $this->Html->link(__l('Edit Role'), array('controller' => 'roles', 'action' => 'edit', $result[$model]['id']), array('class' => 'edit', 'title' => __l('Edit Role')));
			}
			if ($this->__settings['show_delete_role']) {
				$return .= $this->Html->link(__l('Delete Role'), array('controller' => 'roles', 'action' => 'delete', $result[$model]['id']), array('class' => 'delete js-delete', 'title' => __l('Delete Role')));
			}
			if ($this->__settings['show_move_role']) {
				$return .= $this->Html->link(__l('Move Role'), '#', array('class' => 'move', 'title' => __l('Move Role')));
			}
			$return .= '</span></span>';
			/* Suffix */
			$__addType = false;
