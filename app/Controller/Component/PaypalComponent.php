...
class PaypalComponent extends Component
{
    ...
    public function logPaypalTransactions() 
    {
        ...
        $paypalTransactionModel = ClassRegistry::init($this->paypal_transaction_model);
        $_data['PaypalTransactionLog']['paypal_response'] = $this->paypal_response;
        $_data['PaypalTransactionLog']['error_no'] = $this->errno;
        $_data['PaypalTransactionLog']['error_message'] = $errorMessaegString;
        if (!($this->errno == 0)) {
            $_data['PaypalTransactionLog']['paypal_post_vars'] = $this->paypal_post_vars_in_str;
        }
        $_data['PaypalTransactionLog']['ip_id'] = $paypalTransactionModel->toSaveIp();
        foreach($this->paypal_post_arr as $key => $value) {
            $_data['PaypalTransactionLog'][$key] = $value;
        }
        $paypalTransactionModel->save($_data);
