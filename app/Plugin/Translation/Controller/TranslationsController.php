        ...
        if (!empty($this->request->data)) {
            if (!empty($this->request->data['Translation']['googleTranslate']) && !empty($this->request->data['Translation']['language_id'])) {
                ...
                for ($i = 0; $i < count($translations); $i+= 20) {
                    ...
                    $ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, 'https://www.googleapis.com/language/translate/v2?key=' . Configure::read('google.translation_api_key') . '&source=en&target=' . $new_language['Language']['iso2'] . '&' . $key);
